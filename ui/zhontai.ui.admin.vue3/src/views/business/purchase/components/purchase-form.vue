<template>
  <div>
    <el-dialog v-model="state.showDialog" destroy-on-close :title="state.form.id>0?'编辑购货单':'新增购货单'" draggable :close-on-click-modal="false" width="800px">
      <el-form ref="formRef" :model="state.form" label-width="80px">
        ...
        <el-row :gutter="20" class="mt15">
          ...
          <el-col :xs="24" :sm="8"><el-form-item label="本次付款"><el-input-number v-model="state.form.paidAmount" :min="0" :precision="2" style="width:100%" /></el-form-item></el-col>
...
<script lang="ts" setup name="business/purchase/form">
...
const open = async (row: any = {}) => {
  if (row.id > 0) { const res = await new PurchaseApi().get({ id: row.id }); if (res?.data) state.form = res.data as any }
...
const onSure = () => {
  formRef.value!.validate(async (valid: boolean) => { if (!valid) return; state.sureLoading = true
    let res: any
    if (state.form.id > 0) res = await new PurchaseApi().update(state.form).catch(() => { state.sureLoading = false })
    else res = await new PurchaseApi().add(state.form).catch(() => { state.sureLoading = false })
    state.sureLoading = false
    if (res?.success) { ElMessage.success('成功'); eventBus.emit('refreshPurchase'); state.showDialog = false }
  })
}
defineExpose({ open })
</script>
