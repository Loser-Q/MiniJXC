<template>
  <div>
    <el-dialog v-model="state.showDialog" destroy-on-close :title="state.form.id > 0 ? '编辑仓库' : '新增仓库'" draggable
...
<script lang="ts" setup name="business/warehouse/form">
...
const state = reactive({ showDialog: false, sureLoading: false, form: {} as WarehouseEntity })

const open = async (row: any = {}) => {
  if (row.id > 0) { const res = await new WarehouseApi().get({ id: row.id }); if (res?.data) state.form = res.data as any }
...
const onSure = () => {
  formRef.value!.validate(async (valid: boolean) => {
    ...
    let res: any
    if ((state.form as any).id > 0) res = await new WarehouseApi().update(state.form as any).catch(() => { state.sureLoading = false })
    else res = await new WarehouseApi().add(state.form as any).catch(() => { state.sureLoading = false })
    state.sureLoading = false
    if (res?.success) { ElMessage.success('成功'); eventBus.emit('refreshWarehouse'); state.showDialog = false }
  })
}
defineExpose({ open })
</script>
