<template>
  <div class="my-flex-column w100 h100">
    ...
    <transfer-form ref="transferFormRef" />
  </div>
</template>

<script lang="ts" setup name="business/transfer">
import { TransferEntity } from '/@/api/admin/data-contracts'
import { TransferApi } from '/@/api/admin/Transfer'
import eventBus from '/@/utils/mitt'
import { ElMessage, ElMessageBox } from 'element-plus'

const TransferForm = defineAsyncComponent(() => import('./components/transfer-form.vue'))
const transferFormRef = useTemplateRef('transferFormRef')
const state = reactive({ loading: false, total: 0,
  input: { currentPage: 1, pageSize: 20, keyword: '' } as any,
  tableData: [] as TransferEntity[] })
const onQuery = async () => {
  state.loading = true
  const res = await new TransferApi().getPage({ currentPage: state.input.currentPage, pageSize: state.input.pageSize } as any).catch(() => { state.loading = false })
  if (res?.data) { state.tableData = res.data.list ?? []; state.total = res.data.total ?? 0 }
  state.loading = false
}
onMounted(() => { eventBus.off('refreshTransfer'); eventBus.on('refreshTransfer', () => onQuery()); onQuery() })
onBeforeUnmount(() => { eventBus.off('refreshTransfer') })
const onAdd = () => transferFormRef.value!.open({ billDate: new Date().toISOString().split('T')[0], items: [] })
const onEdit = (row: TransferEntity) => transferFormRef.value!.open(row)
const onAudit = async (row: TransferEntity) => {
  await ElMessageBox.confirm(`确定审核调拨单「${row.billNo}」吗？`, '提示', { type: 'warning' })
  const res = await new TransferApi().audit({ id: row.id })
  if (res?.success) { ElMessage.success('审核成功'); onQuery() }
}
const onDelete = async (row: TransferEntity) => {
  await ElMessageBox.confirm(`确定删除「${row.billNo}」吗？`, '提示', { type: 'warning' })
  const res = await new TransferApi().delete({ id: row.id })
