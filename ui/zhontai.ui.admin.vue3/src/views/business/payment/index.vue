...
<script lang="ts" setup name="business/payment">
import { PaymentEntity } from '/@/api/admin/data-contracts'
import { PaymentApi } from '/@/api/admin/Payment'
import eventBus from '/@/utils/mitt'
import { ElMessage, ElMessageBox } from 'element-plus'

const PaymentForm = defineAsyncComponent(() => import('./components/payment-form.vue'))
...
const state = reactive({ loading: false, total: 0,
  ...
  tableData: [] as PaymentEntity[] })
const onQuery = async () => {
  state.loading = true
  const res = await new PaymentApi().getPage({ currentPage: state.input.currentPage, pageSize: state.input.pageSize } as any).catch(() => { state.loading = false })
  if (res?.data) { state.tableData = res.data.list ?? []; state.total = res.data.total ?? 0 }
  state.loading = false
}
onMounted(() => { eventBus.off('refreshPayment'); eventBus.on('refreshPayment', () => onQuery()); onQuery() })
onBeforeUnmount(() => { eventBus.off('refreshPayment') })
const onAdd = () => paymentFormRef.value!.open({ billDate: new Date().toISOString().split('T')[0] })
const onEdit = (row: PaymentEntity) => paymentFormRef.value!.open(row)
const onAudit = async (row: PaymentEntity) => {
  await ElMessageBox.confirm(`确定审核付款单「${row.billNo}」吗？`, '提示', { type: 'warning' })
  const res = await new PaymentApi().audit({ id: row.id })
  if (res?.success) { ElMessage.success('审核成功'); onQuery() }
}
const onDelete = async (row: PaymentEntity) => {
  await ElMessageBox.confirm(`确定删除「${row.billNo}」吗？`, '提示', { type: 'warning' })
  const res = await new PaymentApi().delete({ id: row.id })
