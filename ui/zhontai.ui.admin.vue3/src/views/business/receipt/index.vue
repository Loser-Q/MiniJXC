<template>
  <div class="my-flex-column w100 h100">
    ...
    <receipt-form ref="receiptFormRef" />
  </div>
</template>

<script lang="ts" setup name="business/receipt">
import { ReceiptEntity } from '/@/api/admin/data-contracts'
import { ReceiptApi } from '/@/api/admin/Receipt'
import eventBus from '/@/utils/mitt'
import { ElMessage, ElMessageBox } from 'element-plus'

const ReceiptForm = defineAsyncComponent(() => import('./components/receipt-form.vue'))
const receiptFormRef = useTemplateRef('receiptFormRef')
const state = reactive({ loading: false, total: 0,
  input: { currentPage: 1, pageSize: 20, keyword: '', dateRange: [] as string[] } as any,
  tableData: [] as ReceiptEntity[] })
const onQuery = async () => {
  state.loading = true
  const res = await new ReceiptApi().getPage({ currentPage: state.input.currentPage, pageSize: state.input.pageSize } as any).catch(() => { state.loading = false })
  if (res?.data) { state.tableData = res.data.list ?? []; state.total = res.data.total ?? 0 }
  state.loading = false
}
onMounted(() => { eventBus.off('refreshReceipt'); eventBus.on('refreshReceipt', () => onQuery()); onQuery() })
onBeforeUnmount(() => { eventBus.off('refreshReceipt') })
const onAdd = () => receiptFormRef.value!.open({ billDate: new Date().toISOString().split('T')[0] })
const onEdit = (row: ReceiptEntity) => receiptFormRef.value!.open(row)
const onAudit = async (row: ReceiptEntity) => {
  await ElMessageBox.confirm(`确定审核收款单「${row.billNo}」吗？`, '提示', { type: 'warning' })
  const res = await new ReceiptApi().audit({ id: row.id })
  if (res?.success) { ElMessage.success('审核成功'); onQuery() }
}
const onDelete = async (row: ReceiptEntity) => {
  await ElMessageBox.confirm(`确定删除「${row.billNo}」吗？`, '提示', { type: 'warning' })
  const res = await new ReceiptApi().delete({ id: row.id })
